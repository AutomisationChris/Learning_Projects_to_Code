...
for event in daten['records']:
    ...
    if ausgewählte_tags and not any(tag in ausgewählte_tags for tag in qfap_tags):
        continue
    ...
    with col1:
        if event_pic:
            st.markdown(f'<a href="{event_url}"><img src="{event_pic}" height="200"></a>', unsafe_allow_html=True)
        else:
